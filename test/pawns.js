...
exports.pawn_moves = function(test) {
  ...
  test.equal(board.get_valid_locations("h7").length, 0);

  board.set_fen("8/7p/4K2P/8/4k3/8/8/8 b - - 0 1");
  test.equal(board.get_valid_locations("h6").length, 0);

  board.set_fen("8/8/4K3/pP6/4k3/8/8/8 w - a6 0 1");
  test.deepEqual(board.get_valid_locations("b5").sort(), ["a6", "b6"]);

  board.set_fen("8/8/4K3/6Pp/4k3/8/8/8 w - h6 0 1");
  test.deepEqual(board.get_valid_locations("g5").sort(), ["g6", "h6"]);

  board.set_fen("4k3/8/8/3p4/4P3/8/8/4K3 w - - 0 1");
  test.deepEqual(board.get_valid_locations("e4").sort(), ["d5", "e5"]);
...
exports.en_passant_square_is_a_valid_move = function(test) {
  ...
  board.move("e2", "e4");
  test.deepEqual(board.get_valid_locations("d7").sort(), ["d5", "d6"]);
...
exports.pawn_move_cannot_reveal_check = function(test) {
  ...
  test.deepEqual(board.get_valid_locations("c5").length, 0);
