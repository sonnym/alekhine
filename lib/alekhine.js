...
// ## get_valid_locations
// - from: an array index from the state array
...
Board.prototype.get_valid_locations = function(from) {
  return valid_locations(this._fen, from, true);
}
...
// ## set_position
// set the current position of the board
// - position: String set the position portion of the FEN for the board
Board.prototype.set_position = function(position) {
  var fen_parts = this._fen.split(" ");
  fen_parts[0] = position;
...
Board.prototype.set_fen = function(fen, callback) {
  this._fen = fen;
  this._state = fen2array(fen);
...
// - from_raw String|Number initial position of piece
// - to_raw String|Number initial position of piece
// - callback Function 
Board.prototype.update_state = function(from_raw, to_raw, callback) {
  var from = parseInt(from_raw);
  var to = parseInt(to_raw);

  var piece = this._state[from]
    , valid = valid_locations(this._fen, from, true)
    , capture = (this._state[to] != "")
    , captured = capture ? this._state[to] : null;
  ...
  if ((piece == "p" && to > 55 && to < 64) || (piece == "P" && to >= 0 && to < 8)) {
    if (callback) callback("promote", function(new_piece) {
      piece = new_piece;
      if (this.update_state_helper(piece, from, to, capture) && callback) {
    ...
    if (this.update_state_helper(piece, from, to, capture, callback) && callback) {
      callback("complete", captured);
    }
    else if (callback) {
      callback("fail");
...
Board.prototype.update_state_helper = function(piece, from, to, capture) {
  /* stash storage */
