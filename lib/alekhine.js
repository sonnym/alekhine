...
class Board {
  ...
  // of the form (for the starting position):
  //
  ...
  all_moves() {
    ...
    find_pieces(this.get_state(), this.get_turn(), (piece, position) => {
      const valid = self.get_valid_locations(position2square(position));
  ...
  move(from_square, to_square, callback) {
    const from = square2position(from_square);
    const to = square2position(to_square);
...
function valid_locations(fen, start, check_for_check) {
  ...
  const en_passant = (!fen_parts[3] || fen_parts[3] === "-") ? null : square2position(fen_parts[3]);
  ...
  for (const d in distances) {
    ...
    do {
      ...
      for (const i in indices) {
        ...
        const row_diff = Math.abs(position2row(prev_index) - position2row(index));
