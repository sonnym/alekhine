...
Board.prototype.move = function(from_raw, to_raw, callback) {
  ...
  if ((piece === "p" && to > 55 && to < 64) || (piece === "P" && to >= 0 && to < 8)) {
    ...
    if (callback) {
      callback("promote", function(new_piece) {
...
//
// returns Boolean
function is_state_check(state, whom) {
  var turn = (whom === "w") ? "b" : "w"
  var king_position = null;
  var turn_validator = function (piece, t) {
  ...
  for (var i = 0; i < 64; i++) {
    if (turn_validator(state[i], turn)) {
      var valid = valid_locations(array2fen(state) + " " + turn, i, false);
...
Board.prototype.all_moves = function() {
  ...
  var piece_checker = function(piece) {
    var ascii = piece.charCodeAt(0);
    return (turn === "w" && ascii > 64 && ascii < 91) ||
           (turn === "b" && ascii > 96 && ascii < 123);
  };
  ...
  for (var i = 0; i < 64; i++) {
    ...
    if (piece !== "" && piece_checker(piece)) {
      var valid = this.get_valid_locations(i);
      if (valid.length > 0) {
        piece_moves[i] = { "piece": piece, "valid": valid };
      }
    }
  }
