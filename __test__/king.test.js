...
describe("king moves", () => {
  ...
  it("can move to all surrounding squares", () => {
    ...
    expect(board.get_valid_locations("b6").sort())
      .toEqual(["a5", "a6", "a7", "b5", "b7", "c5", "c6", "c7"])

    board.set_fen("8/8/1K6/8/4k3/8/8/8 b - - 0 1")
    expect(board.get_valid_locations("e4").sort())
  ...
  it("king move prevents castling", () => {
    ...
    board.move("e1", "f1")
    expect(board.get_castling_availability()).toBe("kq")

    board.move("e8", "f8")
    expect(board.get_castling_availability()).toBe("-")
  })

  it("castle squares are not valid if castling unavailable", () => {
    board.set_fen("r3k2r/pppppppp/8/8/8/8/PPPPPPPP/R3K2R w - - 0 1")
    expect(board.get_valid_locations("e1").length).toBe(2)
