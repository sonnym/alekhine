import Board from '../lib/alekhine'

describe("knight moves", () => {
  const board = new Board();

  it("returns the expected values for white to play", () => {
    board.set_fen("7k/8/8/8/8/8/8/1N5K w - - 0 1");
    expect(board.get_valid_locations("b1").sort()).toEqual(["a3", "c3", "d2"])

    board.set_fen("7k/8/8/8/N7/8/8/7K w - - 0 1");
    expect(board.get_valid_locations("a4").sort()).toEqual(["b2", "b6", "c3", "c5"])
  })

  it("cannot move to reveal check", () => {
    board.set_fen("8/3k4/2q5/8/4N3/8/6K1/8 w - - 0 1");
    expect(board.get_valid_locations("e4").length, 0);
  })
})
