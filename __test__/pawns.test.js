...
describe("pawn moves", () => {
  ...
  it("basics", () => {
    ...
    expect(board.get_valid_locations("h7").length).toBe(0)

    board.set_fen("8/7p/4K2P/8/4k3/8/8/8 b - - 0 1")
    expect(board.get_valid_locations("h6").length).toBe(0)

    board.set_fen("8/8/4K3/pP6/4k3/8/8/8 w - a6 0 1")
    expect(board.get_valid_locations("b5").sort()).toEqual(["a6", "b6"])

    board.set_fen("8/8/4K3/6Pp/4k3/8/8/8 w - h6 0 1")
    expect(board.get_valid_locations("g5").sort()).toEqual(["g6", "h6"])

    board.set_fen("4k3/8/8/3p4/4P3/8/8/4K3 w - - 0 1")
    expect(board.get_valid_locations("e4").sort()).toEqual(["d5", "e5"])
  })
  ...
  it("en passant square is a valid move", () => {
    ...
    expect(board.get_valid_locations("d7").sort()).toEqual(["d5", "d6"])
  })
  ...
  it("can promote upon reaching back rank", () => {
    ...
    expect(board.get_fen()).toBe("Q7/8/8/8/8/8/1K3k2/8 b - - 1 1")
  })
  ...
  it("can promote while capturing", () => {
    ...
    expect(board.get_fen()).toBe("1Q6/8/8/8/8/8/1K3k2/8 b - - 0 1")
  })

  it("cannot move to reveal check", () => {
    board.set_fen("8/8/k7/q1P1K3/8/8/8/8 w - - 0 1")
    expect(board.get_valid_locations("c5").length).toBe(0)
